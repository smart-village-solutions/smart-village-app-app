...
import { getNotificationNavigationTarget, getPushNotificationNavigationData } from '../src/helpers';
...
jest.mock('../src/helpers', () => ({
  ...
  getPushNotificationNavigationData: jest.fn()
}));
...
describe('WasteReminderRuntime', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    (getPushNotificationNavigationData as jest.Mock).mockReturnValue(undefined);
  });
  ...
  it('normalizes Android cold-start payloads before resolving their navigation target', () => {
    const normalizedData = { id: 'news-1', query_type: 'NewsItem' };
    (getPushNotificationNavigationData as jest.Mock).mockReturnValue({ data: normalizedData });
    (getNotificationNavigationTarget as jest.Mock).mockReturnValue(navigationTarget);

    act(() => {
      renderer.create(runtime('tab', true));
    });

    const interactionHandler = (usePushNotifications as jest.Mock).mock.calls[0][1];
    const response = {
      notification: {
        request: { content: { body: JSON.stringify(normalizedData), data: {} } }
      }
    };

    expect(interactionHandler(response)).toBe(true);
    expect(getPushNotificationNavigationData).toHaveBeenCalledWith(response);
    expect(getNotificationNavigationTarget).toHaveBeenCalledWith(normalizedData);
  });
